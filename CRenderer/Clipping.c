...
polygon_t create_polygon_from_triangle(vec3_t v0, vec3_t v1, vec3_t v2, tex2_t t0, tex2_t t1, tex2_t t2, vec3_t normal)
{
	polygon_t polygon =
	{
		.vertices = {v0,v1,v2},
...
void triangles_from_polygon(polygon_t* polygon, triangle_t triangles[], int* num_triangles)
{
	for (int i = 0; i < polygon->num_vertices - 2; i++)
	{
		...
		triangles[i].normal = polygon->normal;
...
void clip_polygon_against_plane(polygon_t* polygon, int plane)
{
	...
	// The array of inside vertices that will be part of the final polygon returned via paramter
	...
	while (current_vertex != &polygon->vertices[polygon->num_vertices])
	{
		...
		// If we changed from inside to outside or vice_versa
