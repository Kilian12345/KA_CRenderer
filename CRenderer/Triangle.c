	...
	uint32_t color)
{
	...
	// create vector points and texture coord after we sort vertices
	...
	tex2_t a_uv, tex2_t b_uv, tex2_t c_uv)
{
	...
	// Variables to store the interpolated values of U, V and also 1/W for the current current pixel
	...
	// Adjust 1/W so the pixel taht are closer to the camera have smaller values
	interpolated_reciprocal_w = 1.0 - interpolated_reciprocal_w;


	// Only draw the pixel if the depth value is less that the one already stored in the z_buffer
	// Quick hack, the bug is that Y is supposed to be (Y-1)
	if (x < get_window_width() && y < get_window_height())
	{
		if (interpolated_reciprocal_w < get_zbuffer_at(x,y))
		{
			...
			// Update the z_buffer value with the 1/w of this current pixel
...
void draw_triangle_pixel(int x, int y, uint32_t color, vec4_t point_a, vec4_t point_b, vec4_t point_c)
{
	...
	// Alsointerpolate the value of 1/W for the current pixel
	...
	upng_t* texture)
{
	...
	// Flip the V compoenent to account for inverted UV-coordiantes (V grows downwards)
	v0 = 1.0 - v0;
	v1 = 1.0 - v1;
	v2 = 1.0 - v2;
